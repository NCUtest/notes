# Chapter_1. Brief History
  ...
  * ## Before Calculus
    ...
    * In 1656, John Wallis published his Arithemtica infinitorum, in which he displayed many ideas that were to lead to the integral calculus of Newton, including the famous **Wallis Product** which gives $\pi$,<br>
```math
{2 \over \pi} = {{1 \cdot 3} \over {2 \cdot 2}} \cdot {{3 \cdot 5} \over {4 \cdot 4}} \cdot {{5 \cdot 7} \over {6 \cdot 6}} \cdot ... 
```
      <br>Wallis produced a method to sum infinitesimal quantities and determine the area enclosed by curves.
